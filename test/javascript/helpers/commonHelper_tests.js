...
describe('Common Helper', () => {
  describe('search result status', () => {
    it('returns search result status', () => {
      ...
      expect(searchResultStatus(pageSize, pageNumber, totalCount)).toEqual('Search Results: 1 - 50 of 5,234')
    })
    it('returns search result status after rounding size value equal to total results', () => {
      ...
      expect(searchResultStatus(pageSize, pageNumber, totalCount)).toEqual('Search Results: 5,201 - 5,234 of 5,234')
    })
  })
})
